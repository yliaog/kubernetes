...
func TestEnoughRequests(t *testing.T) {
	...
	for _, test := range enoughPodsTests {
		t.Run(test.name, func(t *testing.T) {
			...
			draManager := dynamicresources.NewDRAManager(ctx, assumecache.NewAssumeCache(logger, informerFactory.Resource().V1beta1().ResourceClaims().Informer(), "resource claim", "", nil), nil, informerFactory)
