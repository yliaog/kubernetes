...
//
// The null value is usable and empty.
type claimStore struct {
	// claims contains all user-owned claims, followed by the special ResourceClaim for extended resources.
	claims []*resourceapi.ResourceClaim
	// numUserOwned is the number of claims without the special ResourceClaim.
	numUserOwned int
}

// init stores the list of user-owned claims and the optional claim owned by the scheduler.
func newClaimStore(claims []*resourceapi.ResourceClaim, extendedResourceClaim *resourceapi.ResourceClaim) claimStore {
	var cs claimStore
	cs.claims = claims
	cs.numUserOwned = len(claims)
...
func (cs claimStore) index(c *resourceapi.ResourceClaim) int {
