...
func TestValidatePodStatusUpdate(t *testing.T) {
	tests := []struct {
		...
		*podtest.MakePod("foo"),
		...
		"empty requestMapping in ExtendedResourceClaimStatus",
	}, {
		*podtest.MakePod("foo",
			podtest.SetContainers(podtest.MakeContainer("ctr", podtest.SetContainerResources(
				podtest.MakeResourceRequirements(
		...
		*podtest.MakePod("foo"),
		"Duplicate value: \"ctr\"",
		"invalid container name and extended resource name in requestMapping in ExtendedResourceClaimStatus",
	}, {
		*podtest.MakePod("foo",
			podtest.SetContainers(podtest.MakeContainer("ctr", podtest.SetContainerResources(
				podtest.MakeResourceRequirements(
		...
		*podtest.MakePod("foo"),
		`Invalid value: "abc": must match the name of an entry in spec.initContainers.name or spec.containers.name`,
		...
		*podtest.MakePod("foo"),
		`Invalid value: "example.com/cpu": must match the extended resource name of an entry in spec.initContainers.resources.requests or spec.containers.resources.requests`,
		...
		*podtest.MakePod("foo"),
		`status.extendedResourceClaimStatus.resourceClaimName: Invalid value: "xyz/abc": a lowercase RFC 1123 subdomain must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')`,
