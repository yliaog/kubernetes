...
// Used for test cases for DRAExtendedResources feature.
func NewBuilderExtendedResource(f *framework.Framework, driver *Driver) *Builder {
	b := &Builder{f: f, driver: driver, useExtendedResourceName: true}
	ginkgo.BeforeEach(b.setUp)
	return b
}

// Used for test cases for DRAExtendedResources feature, when running in a
// cluster with some nodes running DRA driver, others running device plugin.
func NewBuilderSimulateDevicePlugin(f *framework.Framework, driver *Driver) *Builder {
	b := &Builder{f: f, driver: driver, useExtendedResourceName: true, simulateDevicePlugin: true}
	ginkgo.BeforeEach(b.setUp)
	return b
}
...
func (b *Builder) setUp(ctx context.Context) {
	...
	if b.simulateDevicePlugin {
		nodes, err := b.f.ClientSet.CoreV1().Nodes().List(ctx, metav1.ListOptions{})
		framework.ExpectNoError(err, "getting cluster nodes")
		for _, node := range nodes.Items {
			if _, ok := b.driver.Nodes[node.Name]; ok {
				continue
			}
			node.Status.Capacity[v1.ResourceName(ExtendedResourceName(0))] = resource.MustParse("1")
			retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
				if _, err := b.f.ClientSet.CoreV1().Nodes().UpdateStatus(ctx, &node, metav1.UpdateOptions{}); err != nil {
					return fmt.Errorf("update node %s status capacity for extended resource %s: %w", node.Name, ExtendedResourceName(0), err)
				}
				return nil
			})
...
func (b *Builder) tearDown(ctx context.Context) {
	...
	if b.simulateDevicePlugin {
		...
		for _, node := range nodes.Items {
			...
			delete(node.Status.Capacity, v1.ResourceName(ExtendedResourceName(0)))
			retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
				if _, err := b.f.ClientSet.CoreV1().Nodes().UpdateStatus(ctx, &node, metav1.UpdateOptions{}); err != nil {
					return fmt.Errorf("update node %s status capacity:  %w", node.Name, err)
				}
				return nil
			})
