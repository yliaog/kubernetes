...
func extendedResourceClaimStatusEqual(statusA, statusB *api.PodExtendedResourceClaimStatus) bool {
	...
	if len(statusA.Names) != len(statusB.Names) {
		return false
	}
	...
	// But this cannot be guaranteed, so for the sake of correctness in all
	// cases this code here has to check.
	for i := range statusA.Names {
		if statusA.Names[i].ContainerName != statusB.Names[i].ContainerName {
			return false
		}
		if statusA.Names[i].ExtendedResourceName != statusB.Names[i].ExtendedResourceName {
			return false
		}
		if statusA.Names[i].RequestName != statusB.Names[i].RequestName {
			return false
		}
	}
	return true
